# Study Match (스터디/프로젝트 팀원 매칭 플랫폼 백엔드)

## 1. 프로젝트 개요

- **프로젝트명:** Study Match
- **프로젝트 설명:**
  공부나 프로젝트를 함께할 팀원을 구할 수 있는 프로그램입니다. Spring Security와 JWT를 도입해 안전한 회원 인증을 도입하고 JPA를 이용해 사용자와 게시글의 관계를 1대 다수로 설계했습니다.
...
## 7. 코드 리뷰 반영 및 시스템 개선 (Refactoring & API Docs)

초기 개발 완료 후, 코드 리뷰 피드백을 수용하여 시스템의 안정성과 유지보수성을 높이기 위한 대대적인 리팩토링과 문서 자동화 작업을 진행했습니다.

### 🛠️ DTO 아키텍처 개편 및 안전성 확보
* **패키지 분리:** 기존 `AuthDto`, `RecruitDto` 내부 클래스로 관리되던 구조를 해체하고, 데이터의 흐름에 따라 `request`와 `response` 패키지로 명확히 분리하여 도메인 구조를 개선했습니다.
* **Static 취약점 제거 및 Record 도입:** DTO에 사용되었던 `static` 키워드를 제거하여 멀티 스레드 환경에서 발생할 수 있는 동시성 문제(취약점)를 원천 차단했습니다. 더불어 자바 최신 문법인 `record`를 도입하여 불변성(Immutability)을 보장하고, 롬복(Lombok) 의존성을 줄여 코드를 획기적으로 경량화했습니다.

### 🔒 환경 변수(Environmental Variable)를 통한 보안 강화
* `application.yaml` 파일 내에 평문으로 노출되어 있던 데이터베이스 비밀번호와 JWT 시크릿 키를 제거했습니다.
* 대신 `${DB_PASSWORD}`, `${JWT_SECRET}`과 같이 환경 변수를 주입받도록 구조를 변경하여, 깃허브 등 외부 저장소에 민감한 설정값이 노출되는 보안 사고를 방지했습니다.

### 🧹 코드 가독성 개선 (Clean Code)
* Service 및 Controller 계층에서 불필요하게 선언되던 중간 변수들을 제거하고 바로 인라인(Inline) 처리하여 코드의 길이를 단축했습니다. 데이터의 흐름을 한눈에 파악할 수 있도록 가독성을 대폭 끌어올렸습니다.

### 📖 Swagger를 활용한 API 명세서 자동화
* 프론트엔드 개발자와의 원활한 협업을 위해 `springdoc-openapi` 라이브러리를 도입하여 API 명세서를 자동화했습니다.
* **커스텀 Config 설정:** 단순 자동 생성을 넘어 `SwaggerConfig` 클래스를 직접 작성하여 프로젝트 이름과 설명을 명시했습니다. 특히 Swagger UI 내에서 발급받은 JWT 토큰을 입력(`Bearer` 인증)하여 인가 처리가 필요한 API까지 직접 테스트할 수 있도록 환경을 완벽하게 구축했습니다.
